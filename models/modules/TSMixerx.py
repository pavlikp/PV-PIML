"""
Taken from https://github.com/Nixtla/neuralforecast/blob/main/neuralforecast/models/tsmixerx.py and altered to run standalone in our codebase.
...
class TSMixerx(nn.Module):
    ...
    def __init__(
        ...
        **trainer_kwargs
        ...
        self.hist_exog_size = hist_exog_size if hist_exog_size is not None else 0
        self.futr_exog_size = futr_exog_size if futr_exog_size is not None else 0
        self.stat_exog_size = stat_exog_size if stat_exog_size is not None else 0
        ...
        self.out = nn.Linear(
            in_features=ff_dim, out_features=n_series
        )
    ...
    def forward(self, windows_batch):
        ...
        forecast = self.out(x)  #   [B, h, ff_dim] -> [B, h, n_series]
